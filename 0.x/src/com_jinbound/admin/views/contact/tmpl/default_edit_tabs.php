...
foreach ($this->form->getFieldsets() as $name => $fieldset)
{
	...
	if ($template)
	{
		echo JHtml::_('jinbound.addTab', 'jinbound_default_tabs', $name . '_tab', JText::_('COM_JINBOUND_LEAD_FIELDSET_' . $name, true));
	...
	if (!empty($tabs))
	{
		foreach ($tabs as $tab)
		{
			echo JHtml::_('jinbound.addTab', 'jinbound_default_tabs', $tab . '_tab', JText::_('COM_JINBOUND_LEAD_FIELDSET_' . $tab, true));
