...
class JInboundBaseController extends JInboundCompatBaseController
{
    ...
    function ajax()
    {
        ...
        if (file_exists($com_ajax = ($app->isAdmin() ? JPATH_ADMINISTRATOR : JPATH_ROOT) . '/components/com_ajax/ajax.php')) {
            require $com_ajax;
            $app->close();
        }
            ...
            if ($moduleObject->id != 0) {
                ...
                if (is_file($helperFile)) {
                    ...
                    if (method_exists($class, $method . 'Ajax')) {
                        ...
                        $results = new LogicException(JText::sprintf('COM_JINBOUND_AJAX_METHOD_NOT_EXISTS',
                            $method . 'Ajax'), 404);
                    }
                } // The helper file does not exist
                else {
                    $results = new RuntimeException(JText::sprintf('COM_JINBOUND_AJAX_FILE_NOT_EXISTS',
                ...
                $results = new LogicException(JText::sprintf('COM_JINBOUND_AJAX_MODULE_NOT_ACCESSIBLE',
                    'mod_' . $module), 404);
            }
        } /*
        ...
        switch ($format) {
            // JSONinzed
            case 'json' :
                JLoader::register('JResponseJson',
                    JPATH_ADMINISTRATOR . '/components/com_jinbound/libraries/compat/response/json.php');
