        ...
        if (!is_array($files)) {
            ...
            while (list($key, $file) = each($files['lead'])) {
                ...
                if (method_exists('JFilterInput', 'isSafeFile')) {
                    ...
                    if (!$isSafe) {
                        $this->app->enqueueMessage(JText::sprintf('JLIB_FILESYSTEM_ERROR_WARNFS_ERR03', $filename),
                            'error');
                        continue;
                    }
                }
                ...
                $url                  = JUri::root(false) . 'administrator/' . sprintf('index.php?option=plg_system_jinboundfiles&view=file&file=%1$s&contact=%2$d',
                        htmlspecialchars($pfx . $base, ENT_QUOTES, 'UTF-8'), $conversion->contact_id);
