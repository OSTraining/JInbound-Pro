...
class MCAPI
{
     ...
     *                                for Group A - in YYYY-MM-DD HH:II:SS format in <strong>GMT</strong>
     * @param string $schedule_time_b optional -the time to schedule Group B of an A/B Split "schedule" campaign - in
     *                                YYYY-MM-DD HH:II:SS format in <strong>GMT</strong>
     *
     * @return boolean true on success
     */
     ...
     * @return boolean true on success
     */
    public function campaignSendTest(
        $cid,
        $test_emails = array(),
        $send_type = null
    ) {
     ...
     *
     * Field = An <strong>Address</strong> Merge Var. Use <strong>Merge0-Merge30</strong> or the <strong>Custom
     * Tag</strong> you've setup for your merge field - see listMergeVars(). Note, Address fields can still be used
     * with the default operations below - this section is broken out solely to highlight the differences in using the
     * geolocation routines. Valid Op(erations): <strong>geoin</strong> Valid Values: The number of miles an address
     * should be within Extra Value: The Zip Code to be used as the center point
     *
     ...
     *                             tag" as a value. For Google, this should be
     *                             "google"=>"your_google_analytics_key_here". Note that only "google" is currently
     ...
     * sections as keys - for example, use a key of "html_MAIN" to fill in the "MAIN" section of a template. Supported
     * template sections include: "html_HEADER", "html_MAIN", "html_SIDECOLUMN", and "html_FOOTER"
     ...
     *                             against campaignSegmentTest(). Also, "trans" campaigns <strong>do not</strong>
     ...
     *                             last day of a given month. Defaults to the 1st day of the month
     *
     ...
     * autoresponders string offset-dir either "before" or "after" string event optional "signup" (default) to base
     * this on double-optin signup, "date" or "annual" to base this on merge field in the list string event-datemerge
     * optional sort of, this is required if the event is "date" or "annual"
     *
     ...
     *        campaignUpdate() will not allow you to set a segment that includes no members.</li></ul>
     *
     * @section Campaign  Related
     *
     ...
     *                      of the type - rss, auto, trans, etc.
     ...
     *                       Accepts multiples separated by commas when not using exact matching. int folder_id
     *                       optional - only show campaigns from this folder id - get folders using campaignFolders().
     *                       Accepts multiples separated by commas when not using exact matching. int template_id
     *                       optional - only show campaigns using this template id - get templates using templates().
     *                       Accepts multiples separated by commas when not using exact matching. string  status
     *                       optional - return campaigns of a specific status - one of "sent", "save", "paused",
     *                       "schedule", "sending". Accepts multiples separated by commas when not using exact
     *                       matching. string  type optional - return campaigns of a specific type - one of "regular",
     *                       "plaintext", "absplit", "rss", "trans", "auto". Accepts multiples separated by commas when
     *                       not using exact matching. string  from_name optional - only show campaigns that have this
     *                       "From Name" string  from_email optional - only show campaigns that have this "Reply-to
     *                       Email" string  title optional - only show campaigns that have this title string  subject
     *                       optional - only show campaigns that have this subject string  sendtime_start optional -
     *                       only show campaigns that have been sent since this date/time (in GMT) - format is
     *                       YYYY-MM-DD HH:mm:ss (24hr) string  sendtime_end optional - only show campaigns that have
     *                       been sent before this date/time (in GMT) - format is YYYY-MM-DD HH:mm:ss (24hr) boolean
     *                       exact optional - flag for whether to filter on exact values when filtering, or search
     *                       within content for filter values - defaults to true. Using this disables the use of any
     *                       filters that accept multiples.
     * @param int   $start   optional - control paging of campaigns, start results at this campaign #, defaults to 1st
     *                       page of data  (page 0)
     * @param int   $limit   optional - control paging of campaigns, number of campaigns to return with each call,
     ...
     *               (see Returned Fields for description)
     ...
     * string auto_fb_post A comma delimited list of Facebook Profile/Page Ids the campaign was posted to after
     * sending. If not used, blank. boolean auto_footer Whether or not the auto_footer was manually turned on boolean
     * timewarp Whether or not the campaign used Timewarp boolean timewarp_schedule The time, in GMT, that the Timewarp
     * campaign is being sent. For A/B Split campaigns, this is blank and is instead in their schedule_a and schedule_b
     * in the type_opts array array tracking containing "text_clicks", "html_clicks", and "opens" as boolean values
     * representing whether or not they were enabled string segment_text a string marked-up with HTML explaining the
     * segment used for the campaign in plain English array segment_opts the segment used for the campaign - can be
     * passed to campaignSegmentTest() or campaignCreate() array type_opts the type-specific options for the campaign -
     * can be passed to campaignCreate()
     */
     ...
     *          with the GMT offset as they key. Each timezone will contain: int opens opens for this timezone string
     *          last_open the date/time of the last open for this timezone int unique_opens the unique opens for this
     *          timezone int clicks the total clicks for this timezone string last_click the date/time of the last
     *          click for this timezone int unique_opens the unique clicks for this timezone int bounces the total
     *          bounces for this timezone int total the total number of members sent to in this timezone int sent the
     *          total number of members delivered to in this timezone
     */
     ...
     *                      limit set at 1000
     * @param string $since optional pull only messages since this time - use YYYY-MM-DD HH:II:SS format in
     *                      <strong>GMT</strong>
     *
     ...
     * @returnf string type an internal type generally specifying the orginating mail provider - may not be useful
     *          outside of filling report views
     */
     ...
     * @returnf string code An internal code for the region. When this is blank, it indicates we know the country, but
     *          not the region
     * @returnf string name The name of the region, if we have one. For blank "code" values, this will be "Rest of
     *          Country"
     * @returnf int opens The total number of opens that occurred in the country
     */
     ...
     * array statuses an array of statuses recorded inclduing the status, screen_name, status_id, and datetime fields
     * plus an is_retweet flag
     ...
     * recieved it array lines containing detail of the order - product, category, quantity, item cost
     */
     ...
     *                     string  header_data optional - if "header_type" is text, the text to display. if
     *                     "header_type" is "image" a valid URL to an image file. Note that images will be resized to
     *                     be no more than 500x150. Defaults to the Accounts Company Name. boolean    secure optional -
     *                     whether to require a password for the shared report. defaults to "true" string  password
     *                     optional - if secure is true and a password is not included, we will generate one. It is
     *                     always returned. string  to_email optional - optional, email address to share the report
     *                     with - no value means an email will not be sent array   theme  optional - an array
     *                     containing either 3 or 6 character color code values for: "bg_color", "header_color",
     *                     "current_tab", "current_tab_text", "normal_tab", "normal_tab_text", "hover_tab",
     *                     "hover_tab_text" string  css_url    optional - a link to an external CSS file to be included
     *                     after our default CSS (http://vip-reports.net/css/vip.css) <strong>only if</strong> loaded
     *                     via the "secure_url" - max 255 bytes
     *
     ...
     * @returnf string secure_url The URL to the shared report, including the password (good for loading in an IFRAME).
     ...
     */
    public function campaignShareReport(
        $cid,
        $opts = array()
    ) {
        $params         = array();
    ...
    /**
     * Retrieve the list of email addresses that opened a given campaign with how many times they opened - note: this
     ...
     *                     string plugin_id the MailChimp assigned Plugin Id. Get yours by <a
     *                     href="/register.php">registering here</a> array items the individual line items for an order
     *                     using these keys:
     ...
     *                     contain this will be skipped string product_name the product name for the product_id
     *                     associated with this item. We will auto update these as they change (based on product_id)
     *                     int category_id the store's internal Id for the (main) category associated with this
     *                     product. Our testing has found this to be a "best guess" scenario string category_name the
     *                     category name for the category_id this product is in. Our testing has found this to be a
     *                     "best guess" scenario. Our plugins walk the category heirarchy up and send "Root - SubCat1 -
     *                     SubCat4", etc. double qty the quantity of the item ordered double cost the cost of a single
     *                     item (ie, not the extended cost of the line)
     *                     </td></tr></table></div>
     *
     ...
     * @returnf string field_type The "data type" of this merge var. One of: email, text, number, radio, dropdown,
     *          date, address, phone, url, imageurl
     ...
     *                        dropdown, date, address, phone, url, imageurl - defaults to text boolean req optional
     *                        indicates whether the field is required - defaults to false boolean public optional
     *                        indicates whether the field is displayed in public - defaults to true boolean show
     *                        optional indicates whether the field is displayed in the app's list member view -
     *                        defaults to true string default_value optional the default value for the field. See
     *                        listSubscribe() for formatting info. Defaults to blank array choices optional kind of -
     *                        an array of strings to use as the choices for radio and dropdown type fields
     *
     * @return bool true if the request succeeds, otherwise an error will be thrown
     */
    public function listMergeVarAdd(
        $id,
        $tag,
        $name,
        $options = array()
     ...
     * @returnf array groups Array of the grouping options including the "bit" value, "name", "display_order", and
     *          number of "subscribers" with the option selected.
     */
     ...
     * @param int optional $grouping_id The grouping to add the new group to - get using listInterestGrouping() . If
     ...
     * @param int optional $grouping_id The grouping to delete the group from - get using listInterestGrouping() . If
     *                         not supplied, the first grouping on the list is used.
     *
     * @return bool true if the request succeeds, otherwise an error will be thrown
     */
     ...
     *                                  when passed to listUpdateMember() array GROUPINGS Set Interest Groups by
     *                                  Grouping. Each element in this array should be an array containing the "groups"
     *                                  parameter which contains a comma delimited list of Interest Groups to add.
     *                                  Commas in Interest Group names should be escaped with a backslash. ie, ","
     *                                  =&gt; "\," and either an "id" or "name" parameter to specify the Grouping - get
     *                                  from listInterestGroupings() string OPTINIP Set the Opt-in IP fields.
     *                                  <em>Abusing this may cause your account to be suspended.</em> We do validate
     *                                  this and it must not be a private IP address. array MC_LOCATION Set the members
     *                                  geographic location. By default if this merge field exists, we'll update using
     *                                  the optin_ip if it exists. If the array contains LATITUDE and LONGITUDE keys,
     *                                  they will be used. NOTE - this will slow down each subscribe call a bit,
     *                                  especially for lat/lng pairs in sparsely populated areas. Currently our
     *                                  automated background processes can and will overwrite this based on opens and
     *                                  clicks.
     *
     ...
     * string, but we <em>will</em> verify that it looks like a valid URL
     * @param string $email_type        optional email type preference for the email (html, text, or mobile defaults to
     *                                  html)
     ...
     * @param bool   $update_existing   optional flag to control whether a existing subscribers should be updated
     ...
     *                                    Leave blank to keep the existing preference (optional)
     ...
     *                                   for the email address, and "EMAIL_TYPE" for the email type option (html, text,
     *                                   or mobile)
     ...
     */
    public function listBatchSubscribe(
        $id,
        $batch,
        $double_optin = true,
        $update_existing = false,
     ...
     */
    public function listBatchUnsubscribe(
        $id,
        $emails,
        $delete_member = false,
        $send_goodbye = true,
     ...
     * lots of merge data or specific members of a list? If so, checkout the <a href="/api/export">Export API</a>
     *
     * @section List Related
     ...
     *                       href="http://eepurl.com/dwk1">cleaned</a>, updated), defaults to subscribed
     * @param string $since  optional pull all members whose status (subscribed/unsubscribed/cleaned) has changed or
     *                       whose profile (updated) has changed since this date/time (in GMT) - format is YYYY-MM-DD
     *                       HH:mm:ss (24hr)
     * @param int    $start  optional for large data sets, the page number to start at - defaults to 1st page of data
     *                       (page 0)
     * @param int    $limit  optional for large data sets, the number of results to return - defaults to 100, upper
     ...
     * email address, either pending, subscribed, unsubscribed, or cleaned string ip_opt IP Address this address opted
     * in from. string ip_signup IP Address this address signed up from. int member_rating the rating of the
     * subscriber. This will be 1 - 5 as described <a href="http://eepurl.com/f-2P" target="_blank">here</a> string
     * campaign_id If the user is unsubscribed and they unsubscribed from a specific campaign, that campaign_id will be
     * listed, otherwise this is not returned. array lists An associative array of the other lists this member belongs
     * to - the key is the list id and the value is their status in that list. date timestamp The time this email
     * address was added to the list date info_changed The last time this record was changed. If the record is old
     * enough, this may be blank. int web_id The Member id used in our web app, allows you to create a link directly to
     * it array clients the various clients we've tracked the address as using - each included array includes client
     * 'name' and 'icon_url' array static_segments the 'id', 'name', and date 'added' for any static segment this
     * member is in
     */
     ...
     * string campaign_id The campaign id the action was related to, if it exists - otherwise empty (ie, direct unsub
     * from list)
     */
     ...
     * @returnf int other_removes number of non-manual unsubscribes for the list (deletions, empties, soft-bounce
     *          removals)
     */
     ...
     * array clients a record containing the 'client', an 'icon' image url, the 'percent' using the client, and the
     ...
     *                          Our "start from scratch" extremely basic templates
     ...
     * @returnf string preview_image If we've generated it, the url of the preview image for the template. We do out
     *          best to keep these up to date, but Preview image urls are not guaranteed to be available
     ...
     * @returnf bool pending_monthly Whether the account is finishing Pay As You Go credits before switching to a
     *          Monthly plan
     ...
     * @returnf array rewards Rewards details for the account including credits & inspections earned, number of
     *          referals, referal details, and rewards used int referrals_this_month the total number of referrals this
     *          month string notify_on whether or not we notify the user when rewards are earned string notify_email
     *          the email address address used for rewards notifications array credits Email credits earned
     *          "this_month", "total_earned", and "remaining" array inspections Inbox Inspections earned "this_month",
     *          "total_earned", and "remaining" array referrals All referrals, including "name", "email",
     *          "signup_date", and "type" array applied Applied rewards, including "value", "date", "order_id", and
     *          "order_desc"
     */
     ...
     * an existing Campaign Id, or an existing Template Id. Note that this will <b>not</b> save anything to or update
     * any of your lists, campaigns, or templates.
     *
     * @section Helper
     ...
     * @returnf int folder_id Folder Id for the given folder, this can be used in the campaigns() function to filter
     *          on.
     ...
     *                     store_id) string plugin_id the MailChimp assigned Plugin Id. Get yours by <a
     *                     href="/api/register.php">registering here</a> string campaign_id optional the Campaign Id to
     *                     track this order with (see the "mc_cid" query string variable a campaign passes) array items
     *                     the individual line items for an order using these keys:
     *                     <div style="padding-left:30px"><table><tr><td colspan=*>
     *                     int line_num optional the line number of the item on the order. We will generate these if
     *                     they are not passed int product_id the store's internal Id for the product. Lines that do no
     *                     contain this will be skipped string product_name the product name for the product_id
     ...
     * @returnf string type The type of the message - one of lists:new-subscriber, lists:unsubscribes,
     *          lists:profile-updates, campaigns:facebook-likes, campaigns:facebook-comments,
     *          campaigns:forward-to-friend, lists:imports, or campaigns:inbox-inspections
