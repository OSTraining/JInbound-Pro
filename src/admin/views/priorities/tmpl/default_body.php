...
if (!empty($this->items)) :
    foreach ($this->items as $i => $item) :
        ...
        $canEditOwn = $user->authorise('core.edit.own',
                JInbound::COM . '.priority') && $item->created_by == $userId && $canCheckin;
        ?>
        <tr class="row<?php echo $i % 2; ?>">
            ...
            <td class="order">
                <?php if ($canChange) : ?>
                    <?php if ($saveOrder) : ?>
                        <span><?php echo $this->pagination->orderUpIcon($i, isset($this->ordering[0][$orderkey - 1]),
                                'priorities.orderup', 'JLIB_HTML_MOVE_UP', $item->ordering); ?></span>
                        <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,
                                isset($this->ordering[0][$orderkey + 1]), 'priorities.orderdown', 'JLIB_HTML_MOVE_DOWN',
                                $item->ordering); ?></span>
                    <?php endif; ?>
